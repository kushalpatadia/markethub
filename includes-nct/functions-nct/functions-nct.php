...
function sendEmailAddress($to, $subject, $message) {
    ...
    elseif (SITE_URL == "http://localhost/markethub/") {
        ...
        $mail->Port = 587;
        $mail->IsHTML(true);
        $mail->Username = "katto.kp@gmail.com";
        $mail->Password = "kushalkatto";
        /* end localhost */
    }

    $mail->SetFrom("katto.kp@gmail.com", "Markethub");

    $mail->AddReplyTo("katto.kp@gmail.com", "Markethub");
...
function upload_file($file_arr=array(),$data_array=array()){
    ...
    $folder="";
    $data_array['upload_from'] = isset($data_array['upload_from'])?$data_array['upload_from']:'';
    $all_types=$image_dimension=array();
    

    if($data_array['type']=='profileimg'){
        $folder = DIR_UPD; 

        $all_types=array("jpg","jpeg","png","gif");
        //image dimension array start
        $image_dimension[]=array("width"=>"500","height"=>"500");
        $image_dimension[]=array("width"=>"48","height"=>"48");
        $image_dimension[]=array("width"=>"100","height"=>"100");
       
    }else if($data_array['type']=='COMPANY_LOGO'){
        $folder = DIR_IMG.'Products/'; 

        $all_types=array("jpg","jpeg","png","gif");
        
        $image_dimension[]=array("width"=>"400","height"=>"400");
        $image_dimension[]=array("width"=>"48","height"=>"48");
        $image_dimension[]=array("width"=>"100","height"=>"100");
        //$image_dimension[]=array("width"=>"92","height"=>"26");

    }
    else if($data_array['type']=='COMPANY_COVER'){
        $folder = DIR_UPD; 

        $all_types=array("jpg","jpeg","png","gif");
        
        $image_dimension[]=array("width"=>"1140","height"=>"250");
        
    }

    else if($data_array['type']=='BANNER_IMG'){
        $folder = DIR_UPD; 

        $all_types=array("jpg","jpeg","png","gif");
        
        $image_dimension[]=array("width"=>"260","height"=>"604");
        
    }
        
        if(!$file_arr['name']) return false;
    ...
    if (($data_array['upload_from']!='url' && !move_uploaded_file($file_arr['tmp_name'], $uploadfile)) || ($data_array['upload_from']=='url' && !copy($file_arr['tmp_name'], $uploadfile)) ) {
        ...
        if(count($image_dimension)>0){
            ...
            // $getImageDetails=$magicianObj->getImageDetails();
            //$ratio=$getImageDetails[0]/$getImageDetails[1];
           // $crop = ($ratio<1)?"auto":array('crop',  "m");

            for($i=0;$i<count($image_dimension);$i++){ 
               // if(($image_dimension[$i]['width']<$getImageDetails[0]) || $ratio < 0.5 ){
                     
                    resize($uploadfile, $folder.'th'.($i+1).'_'.$file_name, $image_dimension[$i]['width'], $image_dimension[$i]['height']);   
                    //$magicianObj -> resizeImage($image_dimension[$i]['width'],$image_dimension[$i]['height'], 'auto', true);
                    /*$magicianObj -> resizeImage($image_dimension[$i]['width'],$image_dimension[$i]['height'], $crop, true);
                  
                    $magicianObj -> saveImage($folder.'th'.($i+1).'_'.$file_name, 100);
                    $magicianObj -> reset(); */
                /*}else{
                    copy($uploadfile, $folder.'th'.($i+1).'_'.$file_name);
                }*/
            }
        }
        // if($data_array['old_file']!=''){
