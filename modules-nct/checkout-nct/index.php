	...
	if ($status == "go") {
		...
		else
		{
			...
			while ($row=mysqli_fetch_array($res)) {
				$p_id=$row['p_id'];
				$qty=$row['qty'];	
				$query = "INSERT INTO tbl_order_details (order_id,p_id,u_id,qty,price) VALUES ('$order_id','$p_id','$u_id','$qty','$price')";
				$results = mysqli_query($conn,$query);
			}
...
if(isset($_POST['bypaypal']))
{
	...
	if ($status == "go") {
		...
		{
			redirectPage(SITE_MOD.'checkout-nct/paypal-nct.php?add='.base64_encode($address_id));
		}
	}
	else
	{
